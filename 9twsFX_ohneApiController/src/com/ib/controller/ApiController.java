...
//	private ITimeHandler m_timeHandler;
	...
	private SampleController Sa;
	private int x = 0;
	private int intThreadCount = 1;
	private String aa;
...
//	}
	...
	public ApiController( SampleController handler, ILogger inLogger, ILogger outLogger) {
		Sa = handler;
		m_inLogger = inLogger;
		m_outLogger = outLogger;
		this.Sa = handler;
		m_client = new ApiConnection( this, inLogger, outLogger);
		System.err.println("APIController wird verwendet (the big one)");
	}
	
	private void startMsgProcessingThread() {
		final EReaderSignal signal = new EJavaSignal();		
		final EReader reader = new EReader(client(), signal, this);
		x++;
		System.err.println("MsgThread: " + x );
		reader.start();
		
		new Thread(() -> {
			intThreadCount++;
			updateThreadcount(intThreadCount);
            while (client().isConnected()) {
                signal.waitForSignal();
                try {
                    reader.processMsgs();
                } catch (IOException e) {
                	System.out.println("---------------- is dis the error i get????????????????????");
                    error(e);
                }
            }
        }).start();
...
//
	...
	public void connect( String host, int port, int clientId, String connectionOpts ) {
		if(!m_client.isConnected()){
    		...
    		if(aa == null) {
    			aa = Sa.b_clienetId.getText(); 
    			Sa.b_clienetId.setText(String.valueOf(Sa.clientId));
    		}
	        sendEOM();
		}
    }
	...
	public void disconnect() {
		...
		m_connected = false;
		clearShit();											// eigene Methode
		connected(); 												// eigene Methode
		sendEOM();
	}
	...
	public void reqTheAccountUpdates(boolean subscribe) {
		m_client.reqAccountUpdates(subscribe, acc);
	}
	...
	@Override public void updateAccountValue(String tag, String value, String currency, String account) {
		...
		show(account + "  " + tag + ": " + value + currency + " xxxxxxxxxxxxxxx");
	...
	@Override public void accountSummary( int reqId, String account, String tag, String value, String currency) {
		...
		if (handler != null) {
			...
			logLine(tag + ": " + value + " " + currency);
			System.out.println("in accountsumary if statement");
		}
	...
	public void reqMidMarketData(Contract contract, String genericTickList, boolean snapshot, boolean regulatorySnapshot) {
		int reqId = m_reqId++;
	...
	@Override public void tickPrice(int reqId, int tickType, double price, TickAttrib attribs) { //------------------ Tick price ---------
		ITopMktDataHandler handler = m_topMktDataMap.get( reqId);
		if (handler != null) {
			handler.tickPrice( TickType.get( tickType), price, attribs);
//			logLine(String.valueOf(price) + "               tickPrice");		// ergänzt
		}
		logLine(String.valueOf(price) + "               tickPrice");		// ergänzt
		recEOM();
	}
	...
	@Override public void tickGeneric(int reqId, int tickType, double value) {
		...
		logLine(String.valueOf(value) + "              tickGeneric");		// ergänzt
		recEOM();
	}
	...
	@Override public void tickSize(int reqId, int tickType, int size) {
		...
		logLine(String.valueOf(size) + "               tickSize");		// ergänzt
		recEOM();
	}
...
//		logLine(value + "        tickSting");							// ergänzt
		recEOM();
	}
	...
	@Override public void marketDataType(int reqId, int marketDataType) {
		...
		tLine("MaketDataType = " + marketDataType); // löschen wieder
		recEOM();
	}
    ...
    public void reqTheHistoricalData(Contract contract, String endDateTime, String duration, String barSize, String whatToShow, boolean rthOnly, boolean keepUpToDate) {
		if (!checkConnection())
			return;

    	int reqId = m_reqId++;
    	m_client.reqHistoricalData(reqId, contract, endDateTime, duration, barSize, whatToShow, rthOnly ? 1 : 0, 2, keepUpToDate, Collections.emptyList());
		sendEOM();
    }
    ...
    public void reqTheRealTimeBars(Contract contract, String whatToShow, boolean rthOnly) {
		if (!checkConnection())
			return;

    	int reqId = m_reqId++;
    	List<TagValue> realTimeBarsOptions = new ArrayList<>();
    	m_client.reqRealTimeBars(reqId, contract, 0, whatToShow, rthOnly, realTimeBarsOptions);
		sendEOM();
    }
...
//	}
	...
	@Override public void currentTime(long time) {
		tLine(Formats.fmtDate(time * 1000));
		recEOM();
	}
	...
	public void reqThePnL() {
	    if (!checkConnection())
	        return;

	    int reqId = m_reqId++;
	    String modelCode = "";
	    m_client.reqPnL(reqId, acc, modelCode);
	}
    ...
    public void pnl(int reqId, double dailyPnL, double unrealizedPnL, double realizedPnL) {
        ...
        updateShit("dailyPnL", String.valueOf(dailyPnL), "%");
    ...
    public void reqTheTickByTickData(Contract contract, String tickType, int numberOfTicks, boolean ignoreSize) {
    	if (!checkConnection())
            return;
...
//    	System.out.println(s);
    ...
    public void connected() {
    		Sa.b_connected.setSelected(m_client.isConnected());
    		tLine("Connectionstatus :" + String.valueOf(m_client.isConnected()));
    }
    
    public RadioButton getb_connected() {
    	return Sa.b_connected;
    }

	public void reqAccountSummary(int reqId, String group, String tags) {
		m_client.reqAccountSummary(reqId, group, tags);
	}
	
	public void clearLogWindow() {
		Sa.b_logWindow.clear();
	}
	
	public TextArea getBLogWindow() {
		return Sa.b_logWindow;
	}
	
	public void updateShit(String tag, String value, String currency) {
		...
		Platform.runLater(new Runnable() {
            ...
            public void run() {
                switch(tag) {
                case "TotalCashValue":
                	Sa.b_balance.setText(value + " " + currency);
                	break;
                	
                case "dailyPnL":
                	Sa.b_winDaily.setText(value + " " + currency);
                	break;
                	
                default:
                	System.err.println("no case for :  " + tag + " : " + value + " " + currency);
	...
	public void clearShit() {
		clearLogWindow();
		intThreadCount = 1;
		Sa.b_averagePercent.setText("0 %");
		Sa.b_averageROI.setText("0 %");
		Sa.b_clienetId.setText("xx");
		Sa.b_usedStrats_str.setText("Keine in Nutzung");
		Sa.b_usedStrats_Num.setText("0");
		Sa.b_balance.setText(" -- ");
		Sa.b_bestStrat.setText(" -- ");
		Sa.b_worstStrat.setText(" -- ");
		Sa.b_winDaily.setText(" -- ");
		Sa.b_winWeekly.setText(" -- ");
		Sa.b_winMonthly.setText(" -- ");
		Sa.b_threadCount.setText("0");
		Sa.b_bestStrat.setText(" -- ");
		Sa.b_performanceTotal.setText(" -- ");
		Sa.b_cashInPositions.setText(" -- ");
		updateThreadcount(intThreadCount);	
		aa = null;
	}
	...
	public void updateThreadcount(int i) {
		...
		Platform.runLater(new Runnable() {
            ...
            public void run() {
				...
				Sa.b_threadCount.setText(String.valueOf(i));
