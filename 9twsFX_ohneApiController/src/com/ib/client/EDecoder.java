...
// Die Daten einfach mit m_EWrapper an ApiConn oder EWrapper wweitergeben und dort dann das Logwindow beschreiben
// siehe dazu "processCurrentTimeMsg()" Zeile 995

// Oder eine "Pipe" bilden, die die befehle im ersten Thread ausführen soll?
...
class EDecoder implements ObjectInput {
	...
	public EDecoder(int serverVersion, EWrapper callback, EClientMsgSink sink) {
		...
		m_EWrapper = callback;
		System.err.println("EDecoder ohne SA/ApiConn wird verwendet!");
	}
	
	public EDecoder(int serverVersion, ApiController ApiConn, EClientMsgSink sink) {
		m_clientMsgSink = sink;
		m_serverVersion = serverVersion;
		m_EWrapper = ApiConn;
		this.ApiConn = ApiConn;
		if(TA == null) {
			TA = ApiConn.getBLogWindow();
		}
		System.err.println("EDecoder **mit** SA wird verwendet!");
	}
    ...
    public int processMsg(EMessage msg) throws IOException {
    	if (!readMessageToInternalBuf(msg.getStream())) {
    		System.out.println("xxx in processMsg return 0 xxx");
    		return 0;
    	}
...
//    	System.out.println("....     msgId:   " + msgId);
	...
	private void processCurrentTimeMsg() throws IOException {		// -------------------------- 49 ---------------------------------
		/*int version =*/ readInt();
		long time = readLong();
		m_EWrapper.currentTime(time);								// <---- hier !!!
...
//		ApiConn.show(key + " : " + val + cur);
	...
	private void processAccountSummaryMsg() throws IOException {		//-------------------- 63 ---------------------
		/*int version =*/ readInt();
		int reqId = readInt();
		...
		if(currency != null) {
			...
			ApiConn.updateShit(tag, value, currency);
		}
	...
	// write to my own Logwindow
	
	public void bLog(String s) {
		ApiConn.show(s);
	}
	
	public String currency(String s){
		if (s.equals(null)) {
			return null;
		}
		if(s.equalsIgnoreCase("EUR")) {
			s = "€";
			return s;
		}
		else {
			s = "  -> " + s + " <-  currency() in EDecoder integrieren ";
			return s;
		}
