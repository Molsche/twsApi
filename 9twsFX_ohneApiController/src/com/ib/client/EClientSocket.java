...
public class EClientSocket extends EClient implements EClientMsgSink  {
	...
	protected ApiController ApiConn;
	...
	public EClientSocket(EWrapper eWrapper, EReaderSignal signal) {
		...
		ApiConn = (ApiController) eWrapper;
		System.err.println("----------Der kleine EClientSocket wird verwendet");
	}
	
	public EClientSocket(EWrapper eWrapper, EReaderSignal signal, ApiController ApiConn) {
		super(eWrapper, signal);
		this.ApiConn = ApiConn;
		System.err.println("bigger EClientSocket mit ApiConn wird verwendet ---------------------");
	}
	...
	protected synchronized void eConnect(Socket socket) throws IOException {
	    ...
	    // start reader thread
	    EReader reader = new EReader(this, m_signal, ApiConn);
	...
	public synchronized void eConnect( String host, int port, int clientId, boolean extraAuth) {
	    ...
	    try{
	    	...
	    	System.out.println("Error bei Verbindung in eConnect (EClientSocket)");
	    	eDisconnect();
	        connectionError();
	    }
	}
	...
	private synchronized void eDisconnect( boolean resetState ) {
		...
		System.out.println("zz eDisconnect zz");		//----------------------------------------------------- ???
	    // not connected?
	...
	public synchronized boolean isConnected() {
		boolean bool = m_socket != null && m_socket.isConnected() && m_connected;
		return /*m_socket != null && m_socket.isConnected() && m_connected*/ bool;
	}
